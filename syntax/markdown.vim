...
" Match all blockquoted text
syn region markdownBlockquote start=">\%(\s\|$\)" end="$" contained nextgroup=@markdownBlock

syn region markdownCodeBlock start="    \|\t" end="$" contained

" TODO: real nesting
syn region markdownListMarker start="\%(\t\| \{0,4\}\)[-*+]\%(\s\+\S\)\@=" end="$" contained
...
hi def link markdownListMarker            Operator
hi def link markdownBlockquote            Constant
...
execute "hi! htmlBold gui=bold guifg=" . synIDattr(synIDtrans(hlID('Function')), 'fg')
execute "hi! htmlBoldItalic gui=bold,italic guifg=" . synIDattr(synIDtrans(hlID('Function')), 'fg')
execute "hi! htmlItalic gui=italic guifg=" . synIDattr(synIDtrans(hlID('String')), 'fg')
